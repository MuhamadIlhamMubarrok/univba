...
class MenuController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        if ($menu->submenus()->count() > 0) {
            return redirect()->route('menu')->with('error', 'Cannot delete menu with submenus.');
        }

        $menu->delete();

        return redirect()->route('menu')->with('message', 'Menu deleted successfully.')->with("alert-type", 'success');
    }
}
